...
export default class PrivateVoiceRename extends Command {
    ...
    public async exec(message: Message) {
        ...
        if (!persons_channel.permissionsFor(message.guild!.roles.everyone)?.has("CONNECT"))
            return message.channel.send("Your channel is already locked!");
        ...
        if (
            ...
            return message.channel.send("I don't have permission to lock that channel");

        void message.channel.send("Alright! Locking rn...");
        this.client.inhibitedChannels.add(persons_channel.id);
        return persons_channel.updateOverwrite(message.guild!.roles.everyone, {
            VIEW_CHANNEL: false,
            CONNECT: false,
            SPEAK: false,
        });
